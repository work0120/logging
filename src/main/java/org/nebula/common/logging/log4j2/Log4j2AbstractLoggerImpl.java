package org.nebula.common.logging.log4j2;

import org.apache.logging.log4j.Level;
import org.apache.logging.log4j.Logger;
import org.apache.logging.log4j.Marker;
import org.apache.logging.log4j.MarkerManager;
import org.apache.logging.log4j.message.Message;
import org.apache.logging.log4j.message.SimpleMessage;
import org.apache.logging.log4j.spi.AbstractLogger;
import org.apache.logging.log4j.spi.AbstractLoggerWrapper;
import org.nebula.common.logging.Log;
import org.nebula.common.logging.LogFactory;

public class Log4j2AbstractLoggerImpl extends AbstractLogger implements Log {
    
	private static Marker MARKER=MarkerManager.getMarker(LogFactory.MARKER);
	
	private static final String FQCN = Log4j2Impl.class.getName();
	
	private Logger log;
	
	public Log4j2AbstractLoggerImpl(AbstractLogger abstractLogger) {
	    log = new AbstractLoggerWrapper(abstractLogger, abstractLogger.getName(), abstractLogger.getMessageFactory());
	  }

	
	public void trace(String msg) {
		log(MARKER, FQCN, Level.TRACE, new SimpleMessage(msg), null);

	}

	
	public void trace(String msg, Throwable t) {
		log(MARKER, FQCN, Level.TRACE, new SimpleMessage(msg), t);

	}

	
	public void debug(String msg) {
		log(MARKER, FQCN, Level.DEBUG, new SimpleMessage(msg), null);

	}

	
	public void debug(String msg, Throwable t) {
		log(MARKER, FQCN, Level.DEBUG, new SimpleMessage(msg), t);

	}

	
	public void info(String msg) {
		log(MARKER, FQCN, Level.INFO, new SimpleMessage(msg), null);

	}

	
	public void info(String msg, Throwable t) {
		log(MARKER, FQCN, Level.INFO, new SimpleMessage(msg), t);

	}

	
	public void warn(String msg) {
		log(MARKER, FQCN, Level.WARN, new SimpleMessage(msg), null);

	}

	
	public void warn(String msg, Throwable t) {
		log(MARKER, FQCN, Level.WARN, new SimpleMessage(msg), t);

	}

	
	public void error(String msg) {
		log(MARKER, FQCN, Level.ERROR, new SimpleMessage(msg), null);

	}

	
	public void error(String msg, Throwable t) {
		log(MARKER, FQCN, Level.ERROR, new SimpleMessage(msg), t);

	}

	
	public void fatal(String msg) {
		log(MARKER, FQCN, Level.FATAL, new SimpleMessage(msg), null);

	}

	
	public void fatal(String msg, Throwable t) {
		log(MARKER, FQCN, Level.FATAL, new SimpleMessage(msg), t);

	}

	
	public boolean isTraceEnabled() {
		
		return log.isTraceEnabled();
	}

	
	public boolean isDebugEnabled() {
		
		return log.isDebugEnabled();
	}

	
	public boolean isInfoEnabled() {
		
		return log.isInfoEnabled();
	}

	
	public boolean isWarnEnabled() {
		
		return log.isWarnEnabled();
	}

	
	public boolean isErrorEnabled() {
		
		return log.isErrorEnabled();
	}

	
	public boolean isFatalEnabled() {
		
		return log.isFatalEnabled();
	}


	@Override
	protected boolean isEnabled(Level level, Marker marker, Message data,
			Throwable t) {
		// TODO Auto-generated method stub
		return false;
	}


	@Override
	protected boolean isEnabled(Level level, Marker marker, Object data,
			Throwable t) {
		// TODO Auto-generated method stub
		return false;
	}


	@Override
	protected boolean isEnabled(Level level, Marker marker, String data) {
		// TODO Auto-generated method stub
		return false;
	}


	@Override
	protected boolean isEnabled(Level level, Marker marker, String data,
			Object... p1) {
		// TODO Auto-generated method stub
		return false;
	}


	@Override
	protected boolean isEnabled(Level level, Marker marker, String data,
			Throwable t) {
		// TODO Auto-generated method stub
		return false;
	}


	@Override
	public void log(Marker marker, String fqcn, Level level, Message data,
			Throwable t) {
		log.log(level, marker, data, t);
		
	}

}
